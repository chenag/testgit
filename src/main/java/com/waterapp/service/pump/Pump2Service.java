...
import com.waterapp.repository.pump.Pump2Repository;
...
@Service
public class Pump2Service {
    private final Logger log = LoggerFactory.getLogger(getClass());

    @Resource
    private Pump2Repository repository;

    public JSONObject findOneDayPump2(String time){
        String[] times={"00:00:00","04:00:00","08:00:00","12:00:00","16:00:00","20:00:00","23:59:59"};
        JSONObject returnJson=new JSONObject();
        for (int i=0;i<times.length-1;i++){
            List<Pump2> list=repository.findPump2ByDay(time+" "+times[i],time+" "+times[i+1]);
            if (list!=null && list.size()>0){
                Assert.notNull(list, "接口不存在");
            }
            Pump2 model=list.get(0);
