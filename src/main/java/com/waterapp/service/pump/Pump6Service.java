...
import com.waterapp.repository.pump.Pump6Repository;
...
@Service
public class Pump6Service {
    private final Logger log = LoggerFactory.getLogger(getClass());

    @Resource
    private Pump6Repository repository;

    public JSONObject findOneDayPump6(String time){
        String[] times={"00:00:00","04:00:00","08:00:00","12:00:00","16:00:00","20:00:00","23:59:59"};
        JSONObject returnJson=new JSONObject();
        for (int i=0;i<times.length-1;i++){
            List<Pump6> list=repository.findPump6ByDay(time+" "+times[i],time+" "+times[i+1]);
            if (list!=null && list.size()>0){
                Assert.notNull(list, "接口不存在");
            }
            Pump6 model=list.get(0);
